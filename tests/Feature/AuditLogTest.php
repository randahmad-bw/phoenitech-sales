...
class AuditLogTest extends TestCase
{
    ...
    {
        $admin = $this->userWithRole('super_admin');
        $target = $this->userWithRole('sales');

        $this->actingAs($admin, 'sanctum')
            ->putJson("/api/v1/users/{$target->id}", ['roles' => ['general_manager']])
            ->assertStatus(200);
        ...
        $this->assertSame(['sales'], $entry->old_values['roles']);
        $this->assertSame(['general_manager'], $entry->new_values['roles']);
    }
    ...
    {
        // A senior role, but not one holding audit.view — the general manager
        // does hold it, so it would prove nothing here.
        $this->actingAs($this->userWithRole('sales_manager'), 'sanctum')
            ->getJson('/api/v1/audit-logs')
            ->assertStatus(403)
