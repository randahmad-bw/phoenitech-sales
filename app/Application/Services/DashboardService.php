...
class DashboardService
{
     ...
     *
     * The task block is scoped to the reader — the company's board for a
     * manager, their own plate for everybody else — and is absent entirely for
     * an account with no task permission. It is deliberately not a chart: the
     * money on this screen describes a year, while tasks describe today, and
     * the one thing worth interrupting the year with is work that is late.
     */
    public function getData(?int $year = null, ?User $user = null): array
    {
        $year = $year ?? now()->year;

        $tasks = $this->tasks->digest($user);
        ...
        return [
            ...
            'tasks' => $tasks === null ? null : [
                ...
                'focus' => TaskResource::collection($tasks['focus']),
            ],
            'charts' => [
